...
class CrierDeserializerSpec
  ...
  "Deserializer must" must {
    val event = Event(
      ...
      dateTime = 100000000L,
      payload = Some(
        EventPayload.RetrievableContent(
          RetrievableContent(
            id = "0987654321",
    ...
    "properly deserialize a compressed event" in {
      ...
      CrierEventDeserializer.eventFromRecord(
        compressedEventRecord
      ) mustEqual Success(event)
    }
    ...
    "properly deserialize a non-compressed event" in {
      ...
      CrierEventDeserializer.eventFromRecord(eventRecord) mustEqual Success(
        event
      )
    }
  }
